...
class UserUtils private constructor(private val context: Context) {
  ...
  fun getCircularUserIcon(): Drawable {
    ...
    if (bitmapUserIcon == null) {
      val defaultUserIcon =
        UserIcons.getDefaultUserIcon(context.resources, UserHandle.myUserId(), false) as Drawable
      bitmapUserIcon = UserIcons.convertToBitmap(defaultUserIcon)
    }
    val iconSize =
      context.resources.getDimension(com.android.internal.R.dimen.user_icon_size).toInt()
    val drawableUserIcon = CircleFramedDrawable(bitmapUserIcon, iconSize) as Drawable
  ...
  companion object {
    @Volatile private var instance: UserUtils? = null

    fun getInstance(context: Context): UserUtils {
      return instance
        ?: synchronized(this) { instance ?: UserUtils(context).also { instance = it } }
    }
  }
}
